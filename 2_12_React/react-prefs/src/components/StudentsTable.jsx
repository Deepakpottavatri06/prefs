...
const StudentsTable = () => {
  ...
  const fetchStudents = async () => {
    try {
      ...
      const response = await axios.get("http://10.11.14.150:4000/allstudents", {
        headers: {
  ...
  const handleSubmitClick = async (rollNo) => {
    try {
      const response = await axios.put(`
          http://10.11.14.150:4000/student/${rollNo}`,
  ...
  const handleDeleteClick = async (rollNo) => {
    try {
      const response = await axios.delete(`http://10.11.14.150:4000/student/${rollNo}`, {
        headers: {
          "ngrok-skip-browser-warning": true,
        },
      });
      ...
      alert(response.data.message || "Student(s) deleted successfully!");
      fetchStudents(); // Refresh table data
    } catch (error) {
