...
public class Day26P2 {
    ...
    public static int maxGold(int [] gold, int n , int m){
        int low = 1;
        int high = Arrays.stream(gold).max().getAsInt();
        int max = 0;
        // System.out.println(high);
        while (low<=high) {
            int mid = (low+high)/2;
            if(checker(mid,gold, m)){
                low = mid+1;
                max = mid;
            }
            else{
    ...
    public static boolean checker(int limit , int [] gold, int m){
        int count = 0;
    ...
    public static void main(String[] args) {
        ...
        System.out.println(maxGold(gold,n,m));
        cin.close();
    }
}